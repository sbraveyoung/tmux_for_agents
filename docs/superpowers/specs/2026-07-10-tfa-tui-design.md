...
**中英双语 UI 文案（2026-07-12 补记，首次开源发布前）**：本节及全文所有渲染字符串（Header/列表/详情/Footer/nav 错误提示）现经统一的 `Texts` 表（`src/tui/i18n.rs`）渲染，不再是 view 层内嵌的中文字面量。语言解析（`resolve_lang`，纯函数）优先取 `[tui] lang` 配置（`"en"`/`"zh"`，大小写不敏感）；配置缺失或为 `"auto"`（默认）时按 `LC_ALL`/`LC_MESSAGES`/`LANG` 环境变量探测（含 `"zh"` 子串→中文，否则英文），启动时解析一次、运行期不热切换，与既有 `[tui]` 颜色配置的「只读一次」约定同构。`Model.nav_error` 同期从 `Option<String>` 改为 `Option<NavError>`（`NotInTmux`/`TargetGone` 两个变体）——Model 保持纯逻辑/语言无关，具体文案由 view 层按 `Texts` 查表渲染，不把 UI 字符串泄漏进逻辑层。本节所有中文文案示例（`已连接·刚刚`、`重连中…`、`该会话已结束，刷新中…`等）均为 `zh` 语言表的取值；`en` 语言表见 README 与 `i18n.rs` 源码，语义等价、逐字段翻译，不再在本 spec 内重复列出对照表。
...
- **导航错误的语言无关表示（2026-07-12 补记）**：`Err(_)` 分支不再把中文字面量直接塞进 `Model.nav_error`——写入的是 `NavError::TargetGone`（`NotInTmux` 对应 §7.5 的非 tmux 环境分支），Footer 具体展示哪种语言的提示文字由 `tui::view` 按启动时解析好的 `Texts` 决定（见 §6 补记）。行为契约本身（非 0 退出码留在 TUI、下一次快照自然纠正）不变，变的只是「文案存哪一层」。
