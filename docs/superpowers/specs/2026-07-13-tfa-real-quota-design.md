...
- config 不开启（默认）时，**tfa 的行为保持以下精确保证**：零 API 调用、零 Keychain 访问、
  渲染不变；`tfa list` JSON 因快照加法新增两个恒 null 字段（§5 兼容语义）。
...
2. **响应**（statusline CLAUDE.md 记录 + 待任务 1 真机 curl 钉死）：
   ...
   **真机核对注记（2026-07-13，经用户授权单次 curl，HTTP 200）**：实际响应远比社区文档丰富——
   `resets_at` 带微秒（`.370511+00:00`，解析器取前 3 位为 ms）；本账户 `seven_day_sonnet` 为
   `null`（视同缺席）；另有 `seven_day_opus`、`limits[]` 数组（kind/percent/severity/resets_at，
   含 `weekly_scoped` 第三窗口）、`spend`、`extra_usage` 及多个 null 的内部 codename 字段——
   v1 只消费顶层 five_hour/seven_day/seven_day_sonnet，`limits[]`（weekly_scoped 展示）记 backlog。
   响应体脱敏扫描 CLEAN（无账户标识），原样存 `fixtures/oauth_usage.json`。
...
- 任何 UI 永远内联 source 标签（`real_api` / `local_estimate`），两种形态视觉可区分，
  绝不把估算渲染成真实百分比。
- TTL 用单调钟（Instant）计龄，macOS 睡眠期间不走表——TTL 语义是「醒着的 30 分钟」；
  数据年龄标签用 wall-clock 差值如实显示，诚实性不受影响（2026-07-14 终审注记）。
...
- **终审注记（2026-07-14）**：quota_alert 通知文案刻意保持简短、纯英文（`Claude 5h 87%` /
  `resets HH:MM`，见 `AlertArm::evaluate` 实际实现；取代上一条示例里的中文措辞与
  `(en/zh)` 标注），不做本地化——notify 通道层今天整体语言中立，没有按 kind/locale
  路由文案语言的机制：会话类通知（`waiting_input`/`done`/`stale`/`dead`）同样是各自
  硬编码的单一语言。notify 文案的 i18n 不在本里程碑范围内。
...
- 边界注记（Task 4 发现，Task 6 补录）：`real=true` 但尚无 Claude 会话产生过本地
  消耗时，本地估算尚无 Claude 条目可供 `merge()` 覆盖，`quota` 段为空、TUI 无
  对应展示——属预期（有 agent 活动后立即出现），不是 bug。
